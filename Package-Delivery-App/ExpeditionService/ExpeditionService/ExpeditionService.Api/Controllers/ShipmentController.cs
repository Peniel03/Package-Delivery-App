...
namespace ExpeditionService.Api.Controllers
{
    ...
    public class ShipmentController : ControllerBase
    {
        ...
        [HttpGet("{id}")] 
        [ProducesResponseType((int)HttpStatusCode.OK)]
        public async Task<IActionResult> GetShipmentById(string id, CancellationToken cancellationToken)
        {
        ...
        /// </summary>
        /// <param name="shipmentDto">The shipment that we have to update</param>
        /// <param name="CancellationToken">the cancellation token</param>
        /// <returns>OK if the shipment has been updated and bad request if the shipment has not been updated</returns>
        [HttpPut("{id}")]  
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> UpdateShipment(ShipmentDto shipmentDto,CancellationToken CancellationToken)
        {
            var result = await _shipmentService.UpdateAsync(shipmentDto, CancellationToken);
            return Ok(result);  
        }
        ...
        [HttpDelete("{id}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> DeleteShipment(string id, CancellationToken CancellationToken)
        {
